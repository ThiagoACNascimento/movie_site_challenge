...
async function tmdbJson(path: string, init?: RequestInit) {
  ...
  if (!res.ok) 
  {
    ...
    try 
    { 
      const body = await res.json(); msg = body?.status_message || msg; 
    } 
    catch {}
    throw new Error(msg);
  }
...
async function listByCategory(cat: Category) {
  switch (cat) 
  {
    case "popular": 
    {
      ...
      return [...m.results, ...t.results].sort((a, b) => b.popularity - a.popularity);
    }
    case "streaming": 
    {
      const [m, t] = await Promise.all([
        tmdbJson("/discover/movie?with_watch_monetization_types=flatrate&sort_by=popularity.desc"),
        tmdbJson("/discover/tv?with_watch_monetization_types=flatrate&sort_by=popularity.desc"),
      ]);
      return [...m.results, ...t.results].sort((a, b) => b.popularity - a.popularity);
    }
    case "tv": 
    ...
    case "rent": 
    {
      const data = await tmdbJson("/discover/movie?with_watch_monetization_types=rent&sort_by=popularity.desc");
      return data.results;
    }
    case "theater": 
    {
...
function pickYouTubeKey(videos: any[]): string | null {
  ...
  const trailers = videos.filter((v: any) => (v.type || "").toLowerCase() === "trailer");
...
export async function GET(req: NextRequest) {
  ...
  {
    if (!TMDB_API_KEY) 
      return NextResponse.json({ ok: false, error: "TMDB_API_KEY ausente" }, { status: 500 });
    
    const url = new URL(req.url);
    const category = (url.searchParams.get("category") || "popular") as Category;
    const want = Math.max(1, Math.min(12, Number(url.searchParams.get("limit") || 8)));

    const ck = `${category}:${want}`;
    const hit = cacheGet(ck);
    if (hit) 
      return NextResponse.json({ ok: true, items: hit });
    ...
    for (const it of list) 
    {
      ...
      const isMovie = (it.media_type ?? (it.title ? "movie" : "tv")) === "movie";
      ...
      try 
      {
        ...
        out.push({ id, title,  youtubeKey: key, youtubeThumbMax, youtubeThumbHQ });
      } 
      catch {}
    }
