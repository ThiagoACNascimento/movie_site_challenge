...
async function listByCategory(cat: Category) {
  switch (cat) 
  {
    case "streaming": 
    {
      ...
      return mix.sort((a, b) => (Number(b.popularity) || 0) - (Number(a.popularity) || 0));
    }
    case "tv": 
    {
...
export async function GET(req: NextRequest) {
  ...
  {
    if (!TMDB_API_KEY)
      return NextResponse.json({ ok: false, error: "Nao tem chave" }, { status: 500 });
    

    const url = new URL(req.url);
    const category = (url.searchParams.get("category") || "streaming") as Category;
    const want = Math.max(1, Math.min(40, Number(url.searchParams.get("limit") || 20)));

    const ck = `${category}:${want}`;
    const cached = cacheGet(ck);

    if (cached)
      return NextResponse.json({ ok: true, items: cached });
